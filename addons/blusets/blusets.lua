    ...
    if (#args >= 3 and args[2]:any('load')) then
        ...
        ashita.tasks.once(1, (function (d, lst)
            ...
            lst:each(function (v, k)
                ...
                if v ~= nil and v ~= '' then
                    local spell = AshitaCore:GetResourceManager():GetSpellByName(v, 0);
                    if spell ~= nil and spell.Index >= 512 and spell.Index <= 1024 then
                        local id = spell.Index - 512;
                        local attempts = 0;
                        while attempts < 5 do
                            blu.set_spell(k, id);
                            coroutine.sleep(d);
                            
                            local current_spells = blu.get_spells();
                            if current_spells[k] == id then
                                break;
                            end
                            attempts = attempts + 1;
                        end
